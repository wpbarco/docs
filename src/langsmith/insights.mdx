...
<Note>Generating insights over 1,000 threads typically costs \$0.50-\$1.00 in OpenAI API calls. The cost grows linearly in the number of traces and the size of each trace.</Note>
...
| --- | --- | --- |
| last_trace.* | Access data from the most recent trace (i.e. final turn) in a thread | `{{last_trace.inputs}}` `{{last_trace.outputs}}` `{{last_trace.error}}`|
| all_thread_messages |  All messages across all traces in the thread, formatted as a conversation | `{{all_thread_messages}}` |

You can also access nested fields using dot notation. For example, the prompt `"Summarize this: {{last_trace.inputs.foo.bar}}"` will include only the "bar" value within the "foo" value of the last trace's inputs.
