...
TODO link out to tool calling conceptual guide, important to emphasize difference between internal/server side tools
...
response = llm_with_tools.invoke(
    "Hello there!",
    tool_choice="GetWeather"  # Will use weather tool even for greetings
)

# Prevent tool usage
response = llm_with_tools.invoke(
    "What's the weather in NYC?",
    tool_choice="none"  # Won't use any tools
)
```
...
class QueryResponse(BaseModel):
    query_type: Union[WeatherQuery, MathQuery]
