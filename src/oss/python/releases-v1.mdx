...
[message content](/oss/langchain-messages#content) docs for more info.
...
new [agents docs](/oss/langchain-agents) and to the
...
When interacting with the Responses API, `langchain-openai` now defaults to storing
response items in message `content`. This behavior was previously opt-in by specifying
`output_version="responses/v1"` when instantiating `ChatOpenAI`. This was done to
resolve BadRequestErrors that can arise in some multi-turn contexts.

**To restore previous behavior**, set the `LC_OUTPUT_VERSION` environment variable to `v0`,
or specify `output_version="v0"` when instantiating `ChatOpenAI`:
