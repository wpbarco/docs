...
research_subagent = {
    ...
    "tools": [internet_search],
    "model": "gpt-4.1", # Optional override, defaults to main agent model
...
A common reality for agents is that some tool operations may be sensitive and require human approval before execution. Deep Agents includes the [Human-in-the-loop Middleware](/oss/python/langchain/middleware#human-in-the-loop) by default. You can provide a tool interrupt config to `create_deep_agent` which is passed to the `HumanInTheLoopMiddleware`. Check out these docs to learn exactly how you can specify an interrupt_config.
...
`create_deep_agent` is implemented with AgentMiddleware. You can provide additional middleware as well to extend the state_schema, add tools, or implement middleware hooks. You can learn more about AgentMiddleware in the [middleware documentation](/oss/python/langchain/middleware).
...
def get_temperature(city: str) -> str:
  ...
  return f"The weather in {city} is 70 degrees Fahrenheit."
