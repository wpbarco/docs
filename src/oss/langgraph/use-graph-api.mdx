...
:::js
For Zod state schemas, we can defined reducers by calling `.register()` on the schema field with the [metadata registry](https://zod.dev/metadata) that LangGraph uses.
...
<Info>
  [Zod v4](https://zod.dev/v4) is the latest version of Zod, and is the recommended version to use with LangGraph. If you're still using Zod v3, defining reducers looks slightly different.

  <Accordion title="Legacy: Defining state with Zod v3">
    ```typescript
    // Importing this path adds a `.langgraph` property to zod schemas adds
    // graph metadata to the schema. This is not compatible with Zod v4 schemas.
