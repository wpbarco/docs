...
Under the hood, `create_agent()` builds a **graph**-based agent runtime using [LangGraph](/oss/python/langgraph/overview). A graph consists of nodes (steps) and edges (connections) that define how your agent processes information. The agent moves through this graph, executing nodes like the model node (which calls the model), the tools node (which executes tools), or pre/post model hook nodes. Learn more about the [graph API](/oss/python/langgraph/graph-api).
...
<Tip>
To learn more about memory, including how to implement long-term memory that persists across sessions, see [Memory](/oss/python/langgraph/memory).
</Tip>
