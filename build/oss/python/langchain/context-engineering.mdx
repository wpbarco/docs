...
For fixed instructions that don't change, use the [`system_prompt`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent(system_prompt)) parameter:
...
For instructions that depend on context (user profile, preferences, session data), use the [`@dynamic_prompt`](https://reference.langchain.com/python/langchain/middleware/#langchain.agents.middleware.dynamic_prompt) middleware:
