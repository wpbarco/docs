...
This guide provides a quick overview for getting started with Chroma [`vector stores`](/oss/python/concepts/#vectorstores). For detailed documentation of all `Chroma` features and configurations head to the [API reference](https://api.js.langchain.com/classes/langchain_community_vectorstores_chroma.Chroma.html).
...
This guide will also use [OpenAI embeddings](/oss/python/integrations/text_embedding/openai), which require you to install the `@langchain/openai` integration package. You can also use [other supported embeddings models](/oss/python/integrations/text_embedding) if you wish.
...
You can also transform the vector store into a [retriever](/oss/python/concepts/retrievers) for easier usage in your chains. 
...
- [Tutorials: working with external knowledge](/oss/python/tutorials/#working-with-external-knowledge).
- [How-to: Question and answer with RAG](/oss/python/how-to/#qa-with-rag)
- [Retrieval conceptual docs](/oss/python/concepts/retrieval)
