...
The [`interrupt`](https://reference.langchain.com/javascript/TBD) function pauses graph execution and returns a value to the caller. When you call [`interrupt`](https://reference.langchain.com/javascript/TBD) within a node, LangGraph saves the current graph state and waits for you to resume execution with input.

To use [`interrupt`](https://reference.langchain.com/javascript/TBD), you need:
...
When you call [`interrupt`](https://reference.langchain.com/javascript/TBD), here's what happens:

1. **Graph execution gets suspended** at the exact point where [`interrupt`](https://reference.langchain.com/javascript/TBD) is called
...
- The value passed to `Command(resume=...)` becomes the return value of the [`interrupt`](https://reference.langchain.com/javascript/TBD) call
- The node restarts from the beginning of the node where the [`interrupt`](https://reference.langchain.com/javascript/TBD) was called when resumed, so any code before the [`interrupt`](https://reference.langchain.com/javascript/TBD) runs again
...
First, define a tool that uses [`interrupt`](https://reference.langchain.com/javascript/TBD):
...
Sometimes you need to validate input from humans and ask again if it's invalid. You can do this using multiple [`interrupt`](https://reference.langchain.com/javascript/TBD) calls in a loop.
...
When you call [`interrupt`](https://reference.langchain.com/javascript/TBD) within a node, LangGraph suspends execution by raising an exception that signals the runtime to pause. This exception propagates up through the call stack and is caught by the runtime, which notifies the graph to save the current state and wait for external input.

When execution resumes (after you provide the requested input), the runtime restarts the entire node from the beginning—it does not resume from the exact line where [`interrupt`](https://reference.langchain.com/javascript/TBD) was called. This means any code that ran before the [`interrupt`](https://reference.langchain.com/javascript/TBD) will execute again. Because of this, there's a few important rules to follow when working with interrupts to ensure they behave as expected.



### Do not wrap `interrupt` calls in try/catch

The way that [`interrupt`](https://reference.langchain.com/javascript/TBD) pauses execution at the point of the call is by throwing a special exception. If you wrap the [`interrupt`](https://reference.langchain.com/javascript/TBD) call in a try/catch block, you will catch this exception and the interrupt will not be passed back to the graph.

* ✅ Separate [`interrupt`](https://reference.langchain.com/javascript/TBD) calls from error-prone code
...
* 🔴 Do not wrap [`interrupt`](https://reference.langchain.com/javascript/TBD) calls in bare try/catch blocks
...
* ✅ Keep [`interrupt`](https://reference.langchain.com/javascript/TBD) calls consistent across node executions
...
* 🔴 Do not conditionally skip [`interrupt`](https://reference.langchain.com/javascript/TBD) calls within a node
* 🔴 Do not loop [`interrupt`](https://reference.langchain.com/javascript/TBD) calls using logic that isn't deterministic across executions
...
* ✅ Pass simple, JSON-serializable types to [`interrupt`](https://reference.langchain.com/javascript/TBD)
...
* 🔴 Do not pass functions, class instances, or other complex objects to [`interrupt`](https://reference.langchain.com/javascript/TBD)
...
Because interrupts work by re-running the nodes they were called from, side effects called before [`interrupt`](https://reference.langchain.com/javascript/TBD) should (ideally) be idempotent. For context, idempotency means that the same operation can be applied multiple times without changing the result beyond the initial execution.

As an example, you might have an API call to update a record inside of a node. If [`interrupt`](https://reference.langchain.com/javascript/TBD) is called after that call is made, it will be re-run multiple times when the node is resumed, potentially overwriting the initial update or creating duplicate records.

* ✅ Use idempotent operations before [`interrupt`](https://reference.langchain.com/javascript/TBD)
* ✅ Place side effects after [`interrupt`](https://reference.langchain.com/javascript/TBD) calls
...
* 🔴 Do not perform non-idempotent operations before [`interrupt`](https://reference.langchain.com/javascript/TBD)
...
When invoking a subgraph within a node, the parent graph will resume execution from the **beginning of the node** where the subgraph was invoked and the [`interrupt`](https://reference.langchain.com/javascript/TBD) was triggered. Similarly, the **subgraph** will also resume from the beginning of the node where [`interrupt`](https://reference.langchain.com/javascript/TBD) was called.
...
<Note>
    Static interrupts are **not** recommended for human-in-the-loop workflows. Use the [`interrupt`](https://reference.langchain.com/javascript/TBD) method instead.
</Note>
