...
[`Pregel`](https://reference.langchain.com/javascript/TBD) implements LangGraph's runtime, managing the execution of LangGraph applications.

Compiling a [StateGraph](https://reference.langchain.com/javascript/TBD) or creating an [entrypoint](https://reference.langchain.com/javascript/TBD) produces a [`Pregel`](https://reference.langchain.com/javascript/TBD) instance that can be invoked with input.
...
> **Note:** The [`Pregel`](https://reference.langchain.com/javascript/TBD) runtime is named after [Google's Pregel algorithm](https://research.google/pubs/pub37252/), which describes an efficient method for large-scale parallel computation using graphs.
...
* [`BinaryOperatorAggregate`](https://reference.langchain.com/javascript/TBD): stores a persistent value, updated by applying a binary operator to the current value and each update sent to the channel, useful for computing aggregates over multiple steps; e.g.,`total = BinaryOperatorAggregate(int, operator.add)`
...
While most users will interact with Pregel through the [StateGraph](https://reference.langchain.com/javascript/TBD) API or the [entrypoint](https://reference.langchain.com/javascript/TBD) decorator, it is possible to interact with Pregel directly.
...
<Tabs>
    ...
    This example demonstrates how to use the [`BinaryOperatorAggregate`](https://reference.langchain.com/javascript/TBD) channel to implement a reducer.
...
<Tabs>
    ...
    In the [Functional API](/oss/javascript/langgraph/functional-api), you can use an [`entrypoint`](https://reference.langchain.com/javascript/TBD) to create a Pregel application. The `entrypoint` decorator allows you to define a function that takes input and returns output.
