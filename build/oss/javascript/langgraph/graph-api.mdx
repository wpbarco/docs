...
The [`StateGraph`](https://reference.langchain.com/javascript/TBD) class is the main graph class to use. This is parameterized by a user defined `State` object.
...
The [`START`](https://reference.langchain.com/javascript/TBD) Node is a special node that represents the node that sends user input to the graph. The main purpose for referencing this node is to determine which nodes should be called first.
...
The entry point is the first node(s) that are run when the graph starts. You can use the @[`addEdge`] method from the virtual [`START`](https://reference.langchain.com/javascript/TBD) node to the first node to execute to specify where to enter the graph.
...
A conditional entry point lets you start at different nodes depending on custom logic. You can use @[`addConditionalEdges`] from the virtual [`START`](https://reference.langchain.com/javascript/TBD) node to accomplish this.
...
To support this design pattern, LangGraph supports returning [`Send`](https://reference.langchain.com/javascript/TBD) objects from conditional edges. `Send` takes two arguments: first is the name of the node, and second is the state to pass to that node.
...
With [`Command`](https://reference.langchain.com/javascript/TBD) you can also achieve dynamic control flow behavior (identical to [conditional edges](#conditional-edges)):
...
When using [`Command`](https://reference.langchain.com/javascript/TBD) in your node functions, you must add the `ends` parameter when adding the node to specify which nodes it can route to:
...
When returning [`Command`](https://reference.langchain.com/javascript/TBD) in your node functions, you must add return type annotations with the list of node names the node is routing to, e.g. `Command[Literal["my_other_node"]]`. This is necessary for the graph rendering and tells LangGraph that `my_node` can navigate to `my_other_node`.

</Note>

Check out this [how-to guide](/oss/javascript/langgraph/graph-api.md#combine-control-flow-and-state-updates-with-command) for an end-to-end example of how to use [`Command`](https://reference.langchain.com/javascript/TBD).

### When should I use Command instead of conditional edges?

- Use [`Command`](https://reference.langchain.com/javascript/TBD) when you need to **both** update the graph state **and** route to a different node. For example, when implementing [multi-agent handoffs](/oss/javascript/langchain/multi-agent#handoffs) where it's important to route to a different agent and pass some information to that agent.
...
[`Command`](https://reference.langchain.com/javascript/TBD) is an important part of human-in-the-loop workflows: when using `interrupt()` to collect user input, [`Command`](https://reference.langchain.com/javascript/TBD) is then used to supply the input and resume execution via `new Command({ resume: "User input" })`. Check out the [human-in-the-loop conceptual guide](/oss/javascript/langgraph/interrupts) for more information.
