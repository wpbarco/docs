...
The functional API supports [human-in-the-loop](/oss/javascript/langgraph/interrupts) workflows using the [`interrupt`](https://reference.langchain.com/javascript/TBD) function and the `Command` primitive.
...
Note that we've paused with an [`interrupt`](https://reference.langchain.com/javascript/TBD) after `step_1`. The interrupt provides instructions to resume the run. To resume, we issue a [`Command`](/oss/javascript/langgraph/interrupts#resuming-interrupts) containing the data expected by the `human_feedback` task.
...
Given a tool call, our function will [`interrupt`](https://reference.langchain.com/javascript/TBD) for human review. At that point we can either:
