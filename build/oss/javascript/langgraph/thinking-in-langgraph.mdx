...
To enable [human-in-the-loop](/oss/javascript/langgraph/add-human-in-the-loop) with `interrupt()`, we need to compile with a [checkpointer](/oss/javascript/langgraph/persistence) to save state between runs:
...
<CardGroup cols={2}>
  ...
  <Card title="Human input is first-class" icon="user" href="/oss/javascript/langgraph/add-human-in-the-loop">
