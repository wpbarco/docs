...
* **Custom output formatting** – adjust or enrich the subagent's response in code before handing it back to the main agent.
  ...
  * This requires wrapping the result in a [`Command`](https://reference.langchain.com/javascript/TBD) (or equivalent structure) so you can merge custom state with the subagent’s response.
