...
```
See [`initChatModel`](https://reference.langchain.com/javascript/TBD) for more detail, including information on how to pass model [parameters](#parameters).
...
The most straightforward way to call a model is to use [`invoke()`](https://reference.langchain.com/javascript/TBD) with a single message or a list of messages.
...
Calling [`stream()`](https://reference.langchain.com/javascript/TBD) returns an <Tooltip tip="An object that progressively provides access to each item of a collection, in order.">iterator</Tooltip> that yields output chunks as they are produced. You can use a loop to process each chunk in real-time:
...
<Accordion title="Advanced streaming topics">
    <Accordion title='"Auto-streaming" chat models'>
        ...
        Callback events allow LangGraph `stream()` and [`streamEvents()`](https://reference.langchain.com/javascript/TBD) to surface the chat model's output in real-time.
    ...
    <Accordion title="Streaming events">
        ...
        See the [`streamEvents()`](https://reference.langchain.com/javascript/TBD) reference for event types and other details.
...
For more details on batching, see the [reference](https://reference.langchain.com/javascript/TBD).
...
To make tools that you have defined available for use by a model, you must bind them using [`bindTools()`](https://reference.langchain.com/javascript/TBD). In subsequent invocations, the model can choose to call any of the bound tools as needed.
