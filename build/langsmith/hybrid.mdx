...
The **hybrid** model lets you run the [_data plane_](/langsmith/data-plane)—that is, your [LangGraph Server](/langsmith/langgraph-server) deployments and associated agent workloads—in your own cloud, while LangChain hosts and manages the [_control plane_](/langsmith/control-plane) (the LangSmith interface and orchestration services). Hybrid combines the convenience of a managed control plane with the flexibility of self-hosting your own runtime environment and backing stores. When hosting LangSmith in a hybrid model, you authenticate with a [LangSmith API key](/langsmith/create-account-api-key).
