...
* Enhanced features: Add [persistence](/oss/python/persistence), [streaming](/langgraph-platform/streaming), [short and long-term memory](/oss/python/memory) and more to your AutoGen agents.
* Multi-agent systems: Build [multi-agent systems](/oss/python/multi-agent) where individual agents are built with different frameworks.
...
Since we're leveraging LangGraph's [persistence](/oss/python/persistence) features we can now continue the conversation using the same thread ID -- LangGraph will automatically pass previous history to the AutoGen agent:
