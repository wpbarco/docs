...
Assistants build on the LangGraph open source concept of [configuration](/oss/python/graph-api#runtime-context).
...
A **run** is an invocation of an assistant. Each run may have its own input, configuration, and metadata, which may affect execution and output of the underlying graph. A run can optionally be executed on a [thread](/oss/python/persistence#threads).
