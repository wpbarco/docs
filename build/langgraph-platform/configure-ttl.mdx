...
<Tip>
  **Prerequisites**
  This guide assumes familiarity with the [LangGraph Platform](/langgraph-platform/index), [Persistence](/oss/python/persistence), and [Cross-thread persistence](/oss/python/persistence#memory-store) concepts.
</Tip>
...
The LangGraph Platform persists both [checkpoints](/oss/python/persistence#checkpoints) (thread state) and [cross-thread memories](/oss/python/persistence#memory-store) (store items). Configure Time-to-Live (TTL) policies in `langgraph.json` to automatically manage the lifecycle of this data, preventing indefinite accumulation.
